...
path = r''+keyword+'_'+str(maxImages)
...
for i in range(1,int(pages)+1):
    ...
    imgs = soup.select('div.row-masonry-cell img') #요소 선택
    hrefs = soup.select('div.row-masonry-cell')
    
    for index, href in enumerate(hrefs):
        href_ = href.find('a')['href']
        if href_.split('/')[1] == 'photos' or href_.split('/')[1] == 'photos' or href_.split('/')[1] == 'photos':
            print('https://pixabay.com/'+href_)
            driver.get('https://pixabay.com/'+href_)
            sleep(1)

            html_for_HQ = driver.page_source
            soup = BeautifulSoup(html_for_HQ,'html.parser')
            imgs = soup.select('div.inside img') #요소 선택
            ...
            for idx, img in enumerate(imgs):
                ...
                if img.get('srcset')==None:
                    if img.get('data-lazy-srcset') == None:
                        if img.get('data-lazy') == None:
                            pass
                        else:
                            srcset = img.get('data-lazy')    
                    else:
                ...
                if len(srcset):
                    print(srcset)
                    src = str(srcset).split()[-2] #가장 작은 사이즈의 이미지 경로 추출
    ...
    print('finish, lastPage',finish, lastPage)
